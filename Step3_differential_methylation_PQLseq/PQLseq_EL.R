...
pqlseq_EL <- function(CountData, Phenotypes, Covariates=NULL, RelatednessMatrix=NULL, LibSize=NULL, fit.method = "AI.REML", fit.maxiter=500, fit.tol=1e-5, verbose=FALSE, ...) {
        ...
        for(iVar in 1:numVar){
                ...
                for( ierr in c(2:dim(model.matrix(model0))[2])){
                        if(length(unique(model.matrix(model0)[,ierr])) == 1){ # EL: It would be useful to take idx into account here (missing values)
                ...
                if(!redflag){
                        ...
                        coef_names <- gsub("Covariates", "", names(model0$coefficients))
                        coefs <- model1$coefficients
                        names(coefs) <- paste("coef", coef_names, sep="_")

                        std_errors <- sqrt(diag(model1$cov))
                        names(std_errors) <- paste("se", coef_names, sep="_")

                        pvalues <- pchisq( (model1$coefficients/std_errors)^2, 1, lower.tail = F)
                        names(pvalues) <- paste("pvalue", coef_names, sep="_")

                        out_one_site <- t(as.data.frame(c(coefs, std_errors, pvalues)))
                        ...
                        res <- data.frame(numIDV = numAnalysis, h2 = h2, sigma2 = sigma2, converged = converged)
                        res <- cbind(res, out_one_site)
                        rownames(res) <- rownames(CountData)[iVar]

                        resBMM <- rbind(resBMM, res)
        ...
        closeAllConnections()
        print(identical(rownames(resBMM), rownames(CountData)))
        return(resBMM)
}# end BMM

# end function PQLseq_EL
