...
# If you want to use it, you need to look at the M bias files first to know how many lines to skip and how many to read (read length)
...
# files should now contain M-bias file names, one file for each sample (in this example the working directory contains no other .txt files)
...
to_be_trimmed
# This just printed M-bias file names that indicate more trimming is needed. How much? Try different start_positions. In this example, if you start from the 4th base, this prints an empty vector indicating that bases 4:9 are within the normal variation. It's enough to trim away the first three bases.
# For an individual sample, the vectors above_normal_variation and below_normal_variation will also tell you where to trim. 

# This example sample had values outside normal variation at positions 1-3. Other samples had them only at position 1 or 2 (of read 2). For simplicity, first 3 bases were trimmed from the 5' end of read 2 of all samples. The other option is to do different trimmings for different samples.
...
# One base was trimmed from the 3' end of read 2 of all samples (in this project).
